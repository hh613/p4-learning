...
def create_packet_ip_tcp(eth_h, src_ip, dst_ip, sport, dport,proto):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-pkt', help='number of packets', type=int, required=False, default=5000) #指定要发送的数据包数量
    parser.add_argument('--n-hh', help='number of heavy hitters', type=int, required=False, default=10) #指定重击手的数量
    parser.add_argument('--n-sfw', help='number of small flows', type=int, required=False, default=990) #指定小流量（普通流）的数量
    parser.add_argument('--p-hh', help='percentage of packets sent by heavy hitters',type=float, required=False, default=0.95) #指定由重击手发送的数据包的百分比
